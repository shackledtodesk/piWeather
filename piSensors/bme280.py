...
#
# Based on bme280.py from Matt Hawkins
...
class bme280:
  ...
  def readBME280ID(self, addr=DEVICE):
    ...
    REG_ID     = 0xD0
    (chip_id, chip_version) = bme280.bus.read_i2c_block_data(addr, REG_ID, 2)
  ...
  def readBME280All(self, addr=DEVICE):
    ...
    bme280.bus.write_byte_data(addr, 0xF2, 1)
    
    bme280.bus.write_byte_data(addr, REG_CONTROL, control)
    
    # Read blocks of calibration data from EEPROM
    # See Page 22 data sheet
    cal1 = bme280.bus.read_i2c_block_data(addr, 0x88, 24)
    cal2 = bme280.bus.read_i2c_block_data(addr, 0xA1, 1)
    cal3 = bme280.bus.read_i2c_block_data(addr, 0xE1, 7)
    ...
    data = bme280.bus.read_i2c_block_data(addr, REG_DATA, 8)
